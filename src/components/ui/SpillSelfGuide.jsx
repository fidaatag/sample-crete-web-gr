...
import CardSelfGuide from "./CardSelfGuide";
...
const SpillSelfGuide = () => {
  ...
  return (
    <section className="ps-4 py-4 lg:px-20 lg:py-8 flex flex-col gap-5 max-w-xl lg:max-w-none mx-auto">
      ...
      <div
        className="hidden lg:flex flex-row gap-5 overflow-hidden"
        onTouchStart={handleTouchStart}
        onTouchEnd={handleTouchEnd}
      >
        {visibleData.map((catalog, index) => (
          <CardSelfGuide key={index} title={catalog.title} bgImage={catalog.bgImage} category={catalog.category} showCategory={catalog.category.length === 0} desc={catalog.desc}/>
        ))}
      </div>
